...
public class FexClientMod implements ClientModInitializer {
    ...
    // Fungsi Super-Cepat Tukar Elytra
    private static void doElytraSwap(MinecraftClient client) {
        ...
        for (int i = 0; i < 36; i++) {
            net.minecraft.item.ItemStack stack = inv.main.get(i);
            if (stack.isEmpty()) continue;
            
            if (hasElytra) {
                // Cari Chestplate di inventory (Kebal Crash antar-versi)
        ...
        if (targetSlot != -1) {
            // Konversi slot inventory biasa ke slot UI layar
            int screenSlot = targetSlot < 9 ? targetSlot + 36 : targetSlot;
            
            // 3 Langkah Swap Instan (0ms Delay)
    ...
    public void onInitializeClient() {
        ...
        // Daftarkan Tombol Elytra & Freelook (Default: Not Bound / UNKNOWN_KEY)
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (freelookingNow && !isFreelooking) {
                ...
                client.options.setPerspective(Perspective.THIRD_PERSON_BACK); // Paksa ke Third Person
            } else if (!freelookingNow && isFreelooking) {
                isFreelooking = false;
                client.options.setPerspective(originalPerspective); // Kembalikan pandangan
            }
        });
    }
}